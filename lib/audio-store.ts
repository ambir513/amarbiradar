...
const getAbsoluteTrackUrl = (url?: string) => {
  if (!url) {
    return "";
  }
  ...
  persist(
    subscribeWithSelector((set, get) => ({
      ...
      async play() {
        ...
        const queueIndex =
          state.currentQueueIndex === -1 && state.queue.length > 0
            ? 0
            : state.currentQueueIndex;

        const currentTrack =
          state.currentTrack ??
          (queueIndex >= 0 ? state.queue[queueIndex] : state.queue[0]);

        if (!currentTrack) {
          console.warn("[AudioStore] play() called with no track in queue");
          return;
        }

        const currentSrc = $audio.getSource();
        const targetSrc = getAbsoluteTrackUrl(currentTrack.url);
        ...
        if (shouldLoadTrack) {
          ...
          const errorMessage = `Error playing ${currentTrack.title || "track"}`;
